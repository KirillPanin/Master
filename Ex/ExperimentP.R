...
#flagGene=TRUE äëÿ îòáîðà ïðèçíàêîâ. Èñïîëüçóåòñÿ òîëüêî ñ äàííûìè ãåííîé ýêñïðåññèè☺
ExperimentP<-function(data,nfile="result.txt",flagGene=FALSE)
{
  ...
  source("TestEnsembleP.R")
  source("GraphResult.R")
  source("ReadDataSet.R")
  source("RotateClassifier.R")
  ...
  if(any(out))
  {
  #convert nominal to numeric èñïîëüçóåòñÿ òîëüêî â àëãîðèòìå RotateClassifier
  matrF=NULL
  nameF=NULL
  ...
  BTree=seq(10,50,by=10)
  
  simpleclass=c("SimpleTree")
  #simpleclass=NULL
  ensemble=c("RotateCl","AdaBoost","Bagging","RandomForest")
 
  out=TestEnsembleP(matr,matrF,class,gene,simpleclass,ensemble,BTree,flagGene)
  #çàïèñü âûõîäà â ôàéë
