...
TestEnsembleP<-function(matr,matrF,class,gene,simpleclass,ensemble,BTree,flagGene)
{
  ...
  ls<-foreach(icount(num.sample),.combine=cfunT, .export=c('general.fun','wrongly.classified','RotateClassifier','predict.Rotate'), .packages=c('randomForest','adabag','rpart')) %dopar%
  {
